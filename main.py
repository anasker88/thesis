...
def show_activation(st_act, anti_st_act, layer, feature, language, layers):
    lang = ["en", "jp"]
    st_act = st_act[language, layer, :, feature]
    anti_st_act = anti_st_act[language, layer, :, feature]
    n_bins = 50
    ...
    plt.savefig(
        f"{output_dir}/{lang[language]}/layer{layers[layer]}_feature{feature}.png"
    )
    plt.close()
...
with open(f"{output_dir}/output.txt", "w") as f:
    ...
    # テンソルに変換
    st_act_tensor = torch.zeros(2, layer_num, sentences, features)
    anti_st_act_tensor = torch.zeros(2, layer_num, sentences, features)
    for i in range(layer_num):
        st_act_tensor[0, i] = st_act_list[0][i]
        st_act_tensor[1, i] = st_act_list[1][i]
        anti_st_act_tensor[0, i] = anti_st_act_list[0][i]
        anti_st_act_tensor[1, i] = anti_st_act_list[1][i]
    for i in range(2):
        f.write(f"Language: {'en' if i == 0 else 'jp'}\n")
        ...
        for j in range(k):
            ind = inds[j]
            top_k = (ind // features, ind % features)
            f.write(
                f"Top k feature indices by diff: Layer {layers[top_k[0]]}, Feature {top_k[1]}\n"
            )
            show_activation(
                st_act_tensor, anti_st_act_tensor, top_k[0], top_k[1], i, layers
            )
